...
class App {
  ...
  private renderBookItem(book: Book, container: Element): void {
    ...
    const borrowStatus = this.libraryService
      .getBorrowedBooks()
      .find((b) => b.bookName === book.nameBook);
    if (borrowStatus) {
  ...
  private setupBookListeners(): void {
    ...
    if (clearBooksButton) {
      clearBooksButton.addEventListener(
        "click",
        this.handleClearBooks.bind(this),
      );
    }
  }
  ...
  private setupUserListeners(): void {
    ...
    if (clearUsersButton) {
      clearUsersButton.addEventListener(
        "click",
        this.handleClearUsers.bind(this),
      );
    }
  }
  ...
  private handleAddBook(event: Event): void {
    ...
    const nameBook = (
      document.querySelector(".input_name_book") as HTMLInputElement
    )?.value;
    const authorBook = (
      document.querySelector(".input_author_book") as HTMLInputElement
    )?.value;
    const releaseYear = (
      document.querySelector(".input_release_year") as HTMLInputElement
    )?.value;
  ...
  private handleAddUser(event: Event): void {
    event.preventDefault();
    const nameUser = (
      document.querySelector(".input_username") as HTMLInputElement
    )?.value;
    const emailUser = (
      document.querySelector(".input_email") as HTMLInputElement
    )?.value;
  ...
  private handleSaveBorrow(): void {
    const inputIdUser = document.getElementById(
      "input_borrow",
    ) as HTMLInputElement;
    const activeBookButton = document.querySelector(
      ".btnBorrowOff[data-book-name]",
    ) as HTMLElement;
    ...
    if (activeBookButton && inputIdUser) {
      ...
      if (userId && bookName) {
        const isBorrowed = this.libraryService.borrowBook(
          bookName,
          parseInt(userId),
        );
        if (isBorrowed) {
  ...
  private updateUserBorrowedCount(userId: number): void {
    ...
    if (user) {
      ...
      ); //використовується для знаходження HTML-елемента з класом user-item і атрибутом data-user-id, що відповідає userId
      if (userItem) {
