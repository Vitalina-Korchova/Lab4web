export function ValidationBook(): boolean {
  ...
  formElements.forEach(function (formElem) {
    if (formElem instanceof HTMLFormElement) {
      ...
      const releaseYearInput = formElem.querySelector(
        ".input_release_year",
      ) as HTMLInputElement;
      const releaseYearFeedback = formElem.querySelector(
        ".input_release_year + .invalid-feedback",
      ) as HTMLElement;

      if (releaseYearInput && releaseYearFeedback) {
        releaseYearInput.addEventListener("input", function () {
          //зроблено в такий спосіб, щоб помилка висвічувалась при вводі
...
function validateReleaseYear(
  releaseYearInput: HTMLInputElement,
  releaseYearFeedback: HTMLElement,
): boolean {
  ...
  if (releaseYear === "") {
    ...
    !/^\d{4}$/.test(releaseYear) ||
    isNaN(releaseYearNum) ||
    releaseYearNum < 1 ||
    releaseYearNum > currentYear
  ) {
...
export function ValidationUser(): boolean {
  ...
  formElements.forEach(function (formElem) {
    if (formElem instanceof HTMLFormElement) {
      ...
      const emailInput = formElem.querySelector(
        ".input_email",
      ) as HTMLInputElement;
      const emailFeedback = formElem.querySelector(
        ".input_email + .invalid-feedback",
      ) as HTMLElement;
